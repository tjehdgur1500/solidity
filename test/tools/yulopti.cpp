...
public:
	...
	bool runSteps(string _source, string _steps)
	{
		if (!parse(_source))
			return false;
		try
...
Allowed options)",
	...
	if (
		...
		arguments.count("input-file") &&
		(!nonInteractive || arguments.count("steps"))
	)
	{
		YulOpti yulOpti;
		...
		if (arguments.count("steps"))
		{
			...
			try
			{
				OptimiserSuite::validateSequence(sequence);
			}
			catch(std::exception const& _exception)
			{
				cerr << "Invalid optimiser step sequence: " << _exception.what() << endl;
				return 1;
			}
			if (!nonInteractive)
				cout << "----------------------" << endl;
			if (!yulOpti.runSteps(input, sequence))
				return 1;
			disambiguated = true;
		}
		if (!nonInteractive)
